...
async function showProbabilityBasics() {
    try {
        await client.connect();
        const res = await client.query("SELECT id, name FROM topics WHERE name ILIKE '%Probability Basics%'");
        if (res.rows.length === 0) {
            console.log("Topic 'Probability Basics' not found.");
            return;
        }
        ...
        if (patterns.rows.length === 0) {
            ...
            patterns.rows.forEach(p => {
                console.log(`\n- [${p.gemini_generated ? 'AI' : 'Manual'}] ${p.pattern}`);
