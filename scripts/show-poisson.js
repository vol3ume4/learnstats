...
async function showPoisson() {
    try {
        await client.connect();
        const res = await client.query("SELECT id, name FROM topics WHERE name ILIKE '%Poisson%'");
        if (res.rows.length === 0) {
            console.log("Topic 'Poisson Distribution' not found.");
            return;
        }
